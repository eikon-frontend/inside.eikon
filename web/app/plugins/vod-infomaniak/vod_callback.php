...
if ($aOptions['vod_api_callbackKey'] == $_REQUEST['key']) {
  ...
  if (get_option('vod_api_version', false) == 2) {
    ...
    if ($output['action'] == 'process_ok') {
      ...
      // Construct MPD URL in the format: https://play.vod2.infomaniak.com/dash/[folder_uuid]/[video_id]/,[streams],.urlset/manifest.mpd
      $media_streams = array();
      foreach ($output['streams'] as $stream) {
        if (isset($stream['sFileId'])) {
          $media_streams[] = sanitize_text_field($stream['sFileId']);
        }
      }
      $dash_url = '';
      if (!empty($media_streams)) {
        $dash_url = "https://play.vod2.infomaniak.com/dash/{$sFolderUuid}/{$sServerCode}/," . implode(',', $media_streams) . ",.urlset/manifest.mpd";
      }

      // Get share URL for the video
      $oApi = $this->getAPI();
      $sShareURL = $oApi->publishShare($sServerCode, 1, 480, 320);

      $db->insert_video($iVideo, $iFolder, $sFileName, $sServerCode, $sPath, $sExtension, $iDuration, $dUpload, $sFolderUuid, sanitize_text_field($output['streams'][1]['sFileUrl']), sanitize_text_field($output['streams'][1]['sPreviewUrl']), $sShareURL, $dash_url);
    }
  } else {
    ...
    // Construct MPD URL for older API version too
    $media_streams = array();
    if (!empty($_POST['files'])) {
      foreach ($_POST['files'] as $file) {
        if (isset($file['sFileId'])) {
          $media_streams[] = sanitize_text_field($file['sFileId']);
        }
      }
    }
    $dash_url = '';
    ...
    $db->insert_video($iVideo, $iFolder, $sFileName, $sServerCode, $sPath, $sExtension, $iDuration, $dUpload, '', '', '', '', $dash_url);
...
echo esc_html(file_get_contents("php://input"));
die();
