            ...
            <div id="dialog-tab3" class="ui-tabs-panel">
              ...
              ?>
                ...
                <select id="uploadSelectFolder" style="width:550px;" onchange="changeFolderUp1();" onkeyup="changeFolderUp1();">
                  <option value="-1">-- <?php _e("Dossier d'envoi", 'vod_infomaniak'); ?> --</option>
                  <?php
                  if (empty($aFolders)) {
                    ...
                    foreach ($aFolders as $oFolder) {
                      ...
                      echo "<option value='" . $oFolder->iFolder . "'" . $selected . ">" . __('Dossier', 'vod_infomaniak') . ": /" . $oFolder->sPath . " , " . __('Nom', 'vod_infomaniak') . ": " . $oFolder->sName . "</option>";
                      $first = false;
                    }
                  }
                  ?>
    ...
    ?>
      ...
      <div class="submit">
        ...
        <input class="button" type="button" name="Submit"
          value="<?php _e("Synchroniser Videos", 'vod_infomaniak'); ?>"
          onclick="vodSynchroAjax(0,0);" style="display: inline;" / id="synchroButton">
        <div id="importProgress" style="display: inline-block;font-weight:bold;padding: 5px;"></div>
      </div>
      ...
      <form id="updateFilterFolder" name="updateFilterFolder" action="<?php echo $action_url; ?>"
        ...
        <select id="sFolderPath" name="sFolderPath" style="width:550px;">
          <option value="-1">-- <?php _e("Dossier Racine", 'vod_infomaniak'); ?>--</option>
          <?php
          if (empty($aFolders)) {
            echo "<option value='0'>" . __("Aucun dossier disponible", 'vod_infomaniak') . "</option>";
          } else {
            $first = true;
            foreach ($aFolders as $oFolder) {
              if (isset($options['vod_filter_folder']) && !empty($options['vod_filter_folder'])) {
                $selected = ($options['vod_filter_folder'] == $oFolder->sPath) ? ' selected="selected"' : '';
              } else {
                $selected = $first ? ' selected="selected"' : '';
              }
              echo "<option value='" . $oFolder->sPath . "'" . $selected . ">" . __('Dossier', 'vod_infomaniak') . ": /" . $oFolder->sPath . " , " . __('Nom', 'vod_infomaniak') . ": " . $oFolder->sName . "</option>";
              $first = false;
            }
          }
          ?>
      ...
      <form id="updateRightPlugins" name="updateRightPlugins" action="<?php echo $action_url; ?>"
        ...
        <select id="integration_role" name="integration_role">
          ...
          foreach ($roles as $role_value => $role_name) {
            $selected = ($options['vod_right_integration'] == get_role($role_value)->capabilities['level_1']) ? 'selected="selected"' : '';
            printf(
        ...
        <select id="upload_role" name="upload_role">
          ...
          $roles = wp_roles()->get_names();
          foreach ($roles as $role_value => $role_name) {
            $selected = ($options['vod_right_upload'] == get_role($role_value)->capabilities['level_1']) ? 'selected="selected"' : '';
            printf(
              '<option value="%s" %s>%s</option>',
              esc_attr($role_value),
              $selected,
        ...
        <select id="player_role" name="player_role">
          <?php
          foreach ($roles as $role_value => $role_name) {
            $selected = ($options['vod_right_player'] == get_role($role_value)->capabilities['level_1']) ? 'selected="selected"' : '';
            printf(
              '<option value="%s" %s>%s</option>',
              esc_attr($role_value),
              $selected,
        ...
        <select id="playlist_role" name="playlist_role">
          <?php
          foreach ($roles as $role_value => $role_name) {
            $selected = ($options['vod_right_playlist'] == get_role($role_value)->capabilities['level_1']) ? 'selected="selected"' : '';
            printf(
              '<option value="%s" %s>%s</option>',
              esc_attr($role_value),
              $selected,
    ...
    <?php } else { ?>
      <p>
        ...
        <select id="uploadSelectFolder" onchange="changeFolderUp1();" onkeyup="changeFolderUp1();">
          <option value="-1">-- Dossier d'envoi --</option>
          <?php
          if (empty($aFolders)) {
            echo "<option value='0'>" . __("Aucun dossier disponible", 'vod_infomaniak') . "</option>";
          } else {
