...
function eikon_enforce_unique_project_slug_on_save($data, $postarr)
{
  ...
  // We use the ACF 'project_authors' field to determine the slug prefix.
  // We do NOT use the native WP post_author, so the teacher's name doesn't end up in the URL.
    $author_prefix = '';
    $post_id       = $postarr['ID'] ?? 0;

    // Check $_POST first (when saving via WordPress Admin UI)
    if (isset($_POST['acf']['field_68dc5a01b7e10'])) {
        $authors_field = $_POST['acf']['field_68dc5a01b7e10'];
        if (is_array($authors_field)) {
            // Filter out ACF's hidden 'acfcloneindex' row
            $actual_rows = array_filter(array_keys($authors_field), function ($key) {
                return $key !== 'acfcloneindex';
            });
            if (count($actual_rows) === 1) {
                $row_key = reset($actual_rows);
                $student_id = $authors_field[$row_key]['field_68dc5a01b7e11'] ?? 0;
                if ($student_id) {
                    $student = get_userdata((int) $student_id);
                    $author_prefix = eikon_project_author_prefix($student);
                }
            }
        }
    // Fallback to database (when saving programmatically or via REST API if ACF not in payload)
    } elseif ($post_id) {
        $existing_count = (int) get_post_meta($post_id, 'project_authors', true);
        if ($existing_count === 1) {
            $student_id = get_post_meta($post_id, 'project_authors_0_author', true);
            if ($student_id) {
                $student = get_userdata((int) $student_id);
                $author_prefix = eikon_project_author_prefix($student);
            }
    ...
    if (!empty($data['post_title'])) {
        ...
        // Fetch the previous author prefixes (both from ACF and native WP) to strip them out
        $old_author_prefix_from_acf = '';
        $old_author_prefix_from_wp  = '';

        if ($post_id) {
            // Check old ACF prefix
            $existing_count = (int) get_post_meta($post_id, 'project_authors', true);
            if ($existing_count === 1) {
                $old_student_id = get_post_meta($post_id, 'project_authors_0_author', true);
                if ($old_student_id) {
                    $old_student = get_userdata((int) $old_student_id);
                    $old_author_prefix_from_acf = eikon_project_author_prefix($old_student);
                }
            }
            // Check old WP author prefix (to clean up slugs generated by previous logic)
            $old_post = get_post($post_id);
            if ($old_post && !empty($old_post->post_author)) {
                $old_wp_author = get_userdata((int) $old_post->post_author);
                $old_author_prefix_from_wp = eikon_project_author_prefix($old_wp_author);
            }
        }
        ...
        // Strip out old prefixes if they exist
        if (!empty($old_author_prefix_from_acf) && str_starts_with($base_slug, $old_author_prefix_from_acf)) {
            $base_slug = substr($base_slug, strlen($old_author_prefix_from_acf));
        } elseif (!empty($old_author_prefix_from_wp) && str_starts_with($base_slug, $old_author_prefix_from_wp)) {
            $base_slug = substr($base_slug, strlen($old_author_prefix_from_wp));
        }
